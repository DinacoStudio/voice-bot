  ...
  switch (command) {
    ...
    case 'j': {
      ...
      try {
        ...
        return message.reply(`🔊 Подключился к каналу **${voiceChannel.name}**!`);
      } catch (err) {
        console.error(err);
    ...
    case 'tts': {
      ...
      if (!state || !state.connection) {
        return message.reply('❌ Бот не подключен к голосовому каналу. Используйте `!join`!');
      }
      ...
      playerManager.enqueue(message.guild.id, text, config.readAuthorName ? message.member.displayName : null);
    ...
    case 'lang': {
      ...
      if (!newLang) {
        ...
        return message.reply(`🌐 Текущий язык TTS: **${currentLang}**. Изменить: \`!lang <код_языка>\` (например: \`ru\`, \`en\`, \`de\`, \`es\`)`);
      }
    ...
    case 'h': {
      const helpEmbed = {
        ...
        description: 'Бот озвучивает сообщения пользователей в голосовом канале (отлично подходит для тех, у кого выключен микрофон!).',
        fields: [
          { name: '`!join` (`!j`)', value: 'Подключить бота к вашему голосовому каналу.' },
          { name: '`!leave` (`!l`)', value: 'Отключить бота от голосового канала.' },
          { name: '`!say <текст>` (`!s`)', value: 'Принудительно озвучить текст.' },
          { name: '`!skip`', value: 'Пропустить текущее озвучивание.' },
          { name: '`!lang <код>`', value: 'Сменить язык озвучки (например: `ru`, `en`, `ja`).' },
          { name: '`!help`', value: 'Показать эту справку.' },
        ],
        footer: { text: 'Авто-озвучка: Отправьте сообщение в чат, находясь в одном канале с ботом!' }
      };
