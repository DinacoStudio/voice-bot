...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName("pitch")
        .setDescription("Тон голоса: от -50% до +50%")
        .setMinValue(-50)
        .setMaxValue(50),
    )
    .addIntegerOption((option) =>
      option
        .setName("volume")
        .setDescription("Громкость: 20–100%")
        .setMinValue(20)
        .setMaxValue(100),
    ),
...
function buildAdminPanel(guildId) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "Настройки применяются сразу и сохраняются после перезапуска.",
    )
    .addFields(
      { name: "🎙️ Автоозвучка", value: enabled(guild.autoTts), inline: true },
      {
        name: "👤 Имя автора",
        value: enabled(guild.readAuthorName),
        inline: true,
      },
      {
        name: "🔇 Только без микрофона",
        value: enabled(guild.readOnlyMuted),
        inline: true,
      },
      {
        name: "🎙️ Смайлик-реакция",
        value: enabled(guild.reactionEnabled),
        inline: true,
      },
      {
        name: "🚪 Автоотключение",
        value: enabled(guild.autoDisconnect),
        inline: true,
      },
      {
        name: "⏱️ Объединение",
        value: `${guild.mergeDelayMs / 1000} сек. тишины`,
        inline: true,
      },
      {
        name: "📝 Лимит текста",
        value: `${guild.maxTextLength} символов`,
        inline: true,
      },
  ...
  const r1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("tts_admin:autoTts")
      .setLabel("Озвучивать автоматически")
      .setEmoji("🎙️")
      .setStyle(guild.autoTts ? ButtonStyle.Success : ButtonStyle.Secondary),
    new ButtonBuilder()
      .setCustomId("tts_admin:readAuthorName")
      .setLabel("Указывать автора")
      .setEmoji("👤")
      .setStyle(
        guild.readAuthorName ? ButtonStyle.Primary : ButtonStyle.Secondary,
      ),
  );

  const r2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("tts_admin:readOnlyMuted")
      .setLabel("Только заглушенные")
      .setEmoji("🔇")
      .setStyle(
        guild.readOnlyMuted ? ButtonStyle.Primary : ButtonStyle.Secondary,
      ),
    new ButtonBuilder()
      .setCustomId("tts_admin:reactionEnabled")
      .setLabel("Реакция")
      .setEmoji("🎙️")
      .setStyle(
        guild.reactionEnabled ? ButtonStyle.Primary : ButtonStyle.Secondary,
      ),
  );

  const r3 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("tts_admin:autoDisconnect")
      .setLabel("Отключаться автоматически")
      .setEmoji("🚪")
      .setStyle(
        guild.autoDisconnect ? ButtonStyle.Primary : ButtonStyle.Secondary,
      ),
  );
  ...
  const delayRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        [500, 1000, 1500, 2500, 4000].map((value) => ({
          label: `${value / 1000} сек.`,
          value: String(value),
          default: guild.mergeDelayMs === value,
        })),
      ),
  );
  const lengthRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        [100, 200, 350, 500, 1000].map((value) => ({
          label: `${value} символов`,
          value: String(value),
          default: guild.maxTextLength === value,
        })),
      ),
  );
  return { embeds: [embed], components: [r1, r2, r3, delayRow, lengthRow] };
}

function buildUserPanel(userId) {
  const user = settings.getUser(userId);
  const voiceEntry = Object.entries(config.availableVoices).find(
    ([, item]) => item.voice === user.voice,
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "🎵 Тон",
        value: `${user.pitch > 0 ? "+" : ""}${user.pitch}%`,
        inline: true,
      },
      { name: "🔊 Громкость", value: `${user.volume}%`, inline: true },
    );
  const select = (id, placeholder, values, current, suffix = "") =>
    new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId(`tts_user:${id}`)
        .setPlaceholder(placeholder)
        .addOptions(
          values.map((value) => ({
            label: `${value}${suffix}`,
            value: String(value),
            default: Number(current) === Number(value),
          })),
        ),
    );
  const voiceRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId("tts_user:voice")
      .setPlaceholder("🎙️ Выберите голос")
      .addOptions(
        Object.entries(config.availableVoices).map(([key, item]) => ({
          label: item.name,
          value: key,
          default: item.voice === user.voice,
        })),
      ),
  );
  return {
...
async function handleSlashCommand(interaction) {
  if (interaction.isButton() && interaction.customId.startsWith("tts_admin:")) {
    if (
      ...
      return interaction.reply({
        content: "❌ Эта панель доступна только администраторам.",
  ...
  if (
    ...
    interaction.customId.startsWith("tts_admin_select:")
  ) {
    if (
      !interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)
    ) {
    ...
    const key = interaction.customId.split(":")[1];
    settings.updateGuild(interaction.guildId, {
      [key]: Number(interaction.values[0]),
    });
    return interaction.update(buildAdminPanel(interaction.guildId));
  }
  ...
  if (
    interaction.isStringSelectMenu() &&
    interaction.customId.startsWith("tts_user:")
  ) {
    const key = interaction.customId.split(":")[1];
    const rawValue = interaction.values[0];
    const value =
      key === "voice"
        ? config.availableVoices[rawValue]?.voice
        : Number(rawValue);
    if (value === undefined) {
      return interaction.reply({
        content: "❌ Неверная настройка.",
        flags: MessageFlags.Ephemeral,
      });
    }
  ...
  switch (commandName) {
    ...
    case "say": {
      ...
      const authorName = guildSettings.readAuthorName
        ? member.displayName
        : null;
      const success = await playerManager.enqueue(
        guildId,
        cleanText,
        authorName,
        userSettings,
      ...
      if (success) {
        return interaction.editReply(
          `🗣️ Добавлено в очередь озвучки: "${cleanText}"`,
        );
      } else {
        return interaction.editReply(
          "❌ Ошибка при добавлении в очередь озвучки.",
        );
      }
    }
    ...
    case "voice": {
      ...
      const voiceName =
        config.availableVoices[selectedVoice]?.voice || config.defaultVoice;
      settings.updateUser(interaction.user.id, { voice: voiceName });

      return interaction.reply({
        ...buildUserPanel(interaction.user.id),
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    case "settings": {
      ...
      const changes = {};
      if (selectedVoice)
        changes.voice = config.availableVoices[selectedVoice].voice;
      ...
      if (Object.keys(changes).length)
        settings.updateUser(interaction.user.id, changes);
      return interaction.reply({
        ...buildUserPanel(interaction.user.id),
        flags: MessageFlags.Ephemeral,
      });
    ...
    case "admin": {
      if (
        ...
        return interaction.reply({
          content: "❌ Нужно право Administrator.",
          flags: MessageFlags.Ephemeral,
        });
      }
      return interaction.reply({
        ...buildAdminPanel(guildId),
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    case "help": {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "`/say <текст>`",
            value: "Озвучить произвольный текст в канале",
          },
          {
            name: "`/voice <выбор>`",
            value: "Сменить нейросетевой голос (Svetlana, Dmitry, Ava, Google)",
          },
